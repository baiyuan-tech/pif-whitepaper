...
- **權威 NOAEL 擷取層的洞已在 v0.5 填實(仍有殘餘)**:v0.3 時 CIR 報告全文擷取幾乎是空的(PubChem 只回 CIR 通用首頁 URL),靠 EPA ToxValDB 與 read-across 補。v0.5 改為直接對 CIR 入口站抓完整報告 PDF 全文抽取(§9.1.3):截至 2026-08-30 已抽 326 份、1,331 個快取成分列帶有 CIR NOAEL,cascade 第 0 階終於有權威數值可用。**殘餘限制**:42% 的報告只有定性結論(無數值 NOAEL,仍走第 2 階定性權威);12 份掃描件抽不出文字;在用成分約 450 列仍在每 6 小時 25 列的節奏中補齊;每季重查狀態頁只花新報告的錢。
...
| v0.5 | 2026-08-30 | §14.6 第一項「權威 NOAEL 擷取層仍有洞」改寫為已填實(CIR 完整報告全文抽取,見 §9.1.3)並列出殘餘限制。 |
