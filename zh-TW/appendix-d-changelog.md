...
- **第 17 章「標示與宣稱合規引擎」**(zh-TW + en 雙語)— §7 法定標示要素與 §10 認定準則的雙法源分工、Claude Vision tool-use 擷取 21 欄位 + 全成分(保持標籤原始順序)+ 驗證標章(含純圖案)、L0030099 262 條官方詞句語料(附件一 34 / 附件二 15 類 184 / 附件三 8 群 24 / 附件四 20)、CJK 相容字元與跨行限值兩個抽取坑、五態判定(red/amber/pending_data/green/gray)、官方源 VIEWSTATE 指紋陷阱、只提示不阻擋與具結歷程、六個真實假陰性。
- **第 18 章「計費模型與服務履約」**(zh-TW + en 雙語)— 以產出而非席次計價的理由、付費身分跟著餘額走(取代「曾付款即永久付費」)、產品位棘輪制的刻意不對稱、防套利階梯退費(純函式無 DB)、委外履約層與「SA 簽署必綁一份具體報告」約束。
- **第 19 章「營運可觀測性與邊緣拓撲」**(zh-TW + en 雙語)— 易失證據問題、沿用 audit_logs 而不另開表的理由、遙測須用獨立交易、保留期邊界寫在 DB trigger、CDN + L4 分流拓撲下兩種常見 IP 取法皆錯、同一根因造成三種不同傷害(遙測失義 / 防爆破塌成全域鎖 / 簽署證據可偽造)、收斂單一來源與行為驗證。

### 修正

- 第 16 章章末導覽「下一章」由附錄 A 改指第 17 章。
- **英文版 4 章(ch13–ch16)的導覽頁尾字串由 `**Navigation**` 改為 `**Nav**`** —— `assets/pdf/concat.sh` 的剝除規則是 `^\*\*Nav\*\*`,`**Navigation**` 不匹配,導致這 4 條導覽**漏進英文 PDF**。
- 主 / zh-TW / en README 目錄、章數(16→19)、引用版本同步至 v0.4.0。
